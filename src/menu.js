...
const menu = (() => {
  ...
  const createFoodItem = item => {
    ...
    const foodBody = html.createElement({ type: 'div', classes: 'menu-description overflow-break',
  ...
  const createFoodItemGroup = ({ itemClass, headerText, menuItem }) => {
    const container = html.createElement({ type: 'div', classes: `${itemClass} food-items d-flex flex-wrap` });
    const header = html.createElement({ type: 'h1', classes: 'h-title', text: headerText });
  ...
  const create = () => {
    ...
    const drinkContainer = html.createElement({ type: 'div', classes: 'drink-menu food-items d-flex flex-wrap'});
